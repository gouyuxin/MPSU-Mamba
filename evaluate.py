...
from models.mpsu import Collator, MPSUMamba
...
def evaluate(
    ...
    total_ewrmsd = 0.0
    inf_pro_bar = tqdm(total=num_batch, desc="Inference on MPSUMamba", ncols=100, leave=False)
...
if __name__ == "__main__":
    ...
    parse.add_argument(
        "--MPSUMamba_checkpoint", type=str, default="./checkpoints/CP/MPSUMamba_Molecule3D_Random", help="The checkpoint of MPSUMamba."
    )
    ...
    collator = Collator()
    model = MPSUMamba.from_pretrained(args["MPSUMamba_checkpoint"]).to(device)
    print(model.config)
